...
export default function Header() {
  ...
  // Scroll & header background logic
  useEffect(() => {
  ...
  // Cho phép các thành phần khác mở modal booking
  useEffect(() => {
  ...
  const handleClick = () => {
    setClicked(true);
    setShowBooking(true);
    // Sau vài giây có thể trở lại trạng thái bình thường
      ...
      <div className="max-w-[1521px] mx-auto px-4 sm:px-8 lg:px-[125.4px]">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-5 flex-1 justify-end px-[15px]">
            {[
              { to: "/rooms", text: "HỆ THỐNG PHÒNG" },
              { to: "/services", text: "DỊCH VỤ" },
              { to: "/food", text: "ẨM THỰC" },
              { to: "/locations", text: "VỊ TRÍ" },
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <button
              onClick={handleClick}
              className={`flex px-4 py-2 justify-center text-b items-center rounded transition-colors duration-300 whitespace-nowrap font-['Be_Vietnam_Pro'] text-sm font-normal leading-6
              ${
                isHomePage && !scrolled
                  ? `border border-white/70 font-semibold drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)]
                    ${clicked ? "text-[#48dfb2]" : "text-white"}
                    hover:text-teal-500 hover:bg-white/10`
                  : `shadow-sm
                    ${clicked ? "text-[#48dfb2] bg-[#48dfb2]/10" : "bg-[#48dfb2] text-white"}
                    hover:bg-[#bff2bc] hover:text-black`
              }`}
            >
              Đặt ngay
            </button>
